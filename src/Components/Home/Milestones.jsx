...
const Milestones = () => {
  ...
  return (
    ...
    >
        <ReadingHeading text="BALCO Chronicles" className="text-white text-center mt-30" />
      <div
        ref={trackRef}
        className="flex h-[70%] items-center px-20 gap-0"
        style={{ width: `${milestones.length * 35}vw` }}
      >
          ...
          return (
            <div
              ...
              className={`
                ...
                p-8 w-[60vw] max-w-[500px] min-w-[380px] h-[45vh]
                text-white bg-transparent shrink-0 flex flex-col justify-center items-center text-center
                ${isEven ? 'mt-0 mb-10' : 'mt-10 mb-0'}
              `}
            >
              {/* Connecting line to the next box */}
              {index !== milestones.length - 1 && (
                <div className="absolute right-0 top-1/2 w-14 transform translate-y-[-50%]" >
                    <img src="/home/arrow-right.svg" alt="" />
                    </div>
              )}

              <h3 className="text-5xl font-bold text-[#74bf44] mb-2">{item.year}</h3>
              {/* <h4 className="text-xl font-semibold mb-3">{item.title}</h4> */}
              <p className="text-2xl text-white/70 w-[80%]">{item.description}</p>
            </div>
          );
        })}
      </div>
