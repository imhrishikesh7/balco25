...
export default function BalancedMegaMenu() {
  ...
  return (
    <div className="sticky top-0 z-50 font-sans bg-white shadow-sm">
      {/* Top Navbar */}
      <div className="mx-auto max-w-[99%] mt-1 bg-white border border-gray-200 text-black rounded-xl px-6 py-6 flex justify-between items-center">
        ...
        <div className="relative w-6 h-6 cursor-pointer">
          <button
            ...
            className="w-full h-full flex flex-col justify-center items-start gap-2 relative z-10"
          >
            <span className={`h-1 bg-black rounded transition-all duration-400 absolute left-0 ${menuOpen ? 'rotate-45 top-1/2 w-full' : 'top-1 w-1/2'}`} />
            <span className={`h-1 bg-black rounded transition-all duration-400 absolute left-0 ${menuOpen ? 'scale-x-0 top-1/2 w-3/4' : 'top-1/2 w-3/4'}`} />
            <span className={`h-1 bg-black rounded transition-all duration-400 absolute left-0 ${menuOpen ? '-rotate-45 top-1/2 w-full' : 'bottom-0 w-full'}`} />
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="absolute left-0 right-0 mx-auto max-w-[99%] mt-2 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden"
          >
            <div className="grid grid-cols-1 md:grid-cols-3">
                    ...
                    <div
                      ...
                      className={`cursor-pointer p-4 rounded-lg border transition-all ${
                        selected.title === menu.title
                          ? 'bg-gradient-to-r from-[#3594cb] to-[#86d85a] text-white'
              ...
              <div className="md:col-span-2 p-8 bg-white">
                ...
                >
                  <h3 className="text-xl font-semibold mb-3 text-black">{selected.title}</h3>
                  {/* <p className="text-sm text-gray-600 mb-6">{selected.subtitle}</p> */}
                      ...
                      return (
                        <Link
                          ...
                          className={`p-4 rounded-lg border transition text-sm ${
                            isActive(item)
                              ? 'bg-gradient-to-br from-[#3594cb] to-[#86d85a] text-white'
                              : 'bg-gray-100 text-black hover:bg-gray-200 border-gray-200 hover:border-black'
                          }`}
                        >
                          {item}
                        </Link>
